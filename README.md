...
基于 [Open-AutoGLM](https://github.com/zai-org/Open-AutoGLM) 框架构建的安卓 AI 助手应用。支持**语音唤醒 + 自定义快捷指令（Skill）**模式，说出指令让 AI 替你操作手机，无需手动点点点。

---

## 这是什么

AutoGLM Android Assistant 是一款结合**语音唤醒 + 快捷指令（Skill）**的 AI 手机自动化助手。

两种核心模式：

**模式一：语音唤醒**
1. 说出唤醒词（如"小爱同学"）
2. 说出任务（如"帮我查一下京东 iPhone 16 最低价"）
3. AI 自动操作，结果反馈给你

**模式二：快捷指令（Skill）**
- 预设常用任务模板（如"比较 {商品A} 和 {商品B} 的价格"）
- 首页一键触发，省去每次输入
- 配合意图识别：语音唤醒时自动匹配到对应快捷指令并填充参数
...
在设置中配置常用任务模板（类似 Skill），首页一键触发，无需每次输入完整指令。

支持**意图识别路由**：开启意图识别（设置 → 意图识别）后，语音唤醒时说出的自然语言会自动与快捷指令语义匹配，匹配成功则直接使用该快捷指令的模板执行。例如说"帮我比一下苹果和华为手机价格"，会自动路由到"比价"快捷指令并填充参数。
