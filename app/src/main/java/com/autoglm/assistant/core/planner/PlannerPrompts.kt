...
你可以灵活选择输出格式：
- 自然语言描述任务
- 或者用JSON格式分解任务（如果任务复杂）

如果选择JSON分解，格式如下：

```json
{
  "analysis": "对整个任务的分析和理解",
  "estimated_duration": 60,
  "sub_tasks": [
    {
      "index": 1,
      "goal": "子任务要达成的目标",
      "current_state": "执行此子任务前的预期状态描述",
      "actions": "UI Agent应该执行的具体操作（清晰、明确、可执行）",
      "context": "相关的上下文信息、注意事项、特殊条件等",
      "dependencies": []
    }
  ]
}```
...
好的规划：
{
  "analysis": "用户的指令已经很明确：在美团外卖平台上订购星巴克的拿铁咖啡，并且有定制需求（少冰少糖）。需要依次完成：打开美团外卖 → 搜索星巴克门店 → 在商品列表中找到拿铁 → 选择少冰少糖选项 → 提交订单（提醒用户支付）。",
...
Please output task planning strictly in the following JSON format:

```json
{
  "analysis": "Analysis and understanding of the entire task",
  "estimated_duration": 60,
  "sub_tasks": [
    {
      "index": 1,
      "goal": "The goal to achieve for this subtask",
      "current_state": "Expected state description before executing this subtask",
      "actions": "Specific operations the UI Agent should perform (clear, explicit, executable)",
      "context": "Relevant context, precautions, special conditions, etc.",
      "dependencies": []
    }
  ]
}```
...
Good planning:
{
  "analysis": "User needs to order a Starbucks latte on Meituan Delivery with customization (less ice, less sugar). Need to open Meituan, search Starbucks, find latte, add customization options, then place order.",
  "estimated_duration": 120,
  "sub_tasks": [
    {
      "index": 1,
